...
export function SpaceDust({ count }) {
  ...
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      const time = Random.range(0, 80);
      const factor = Random.range(20, 120);
      const speed = Random.range(0.006, 0.015) / 8;
  ...
  return (
    <>
      <pointLight ref={light} distance={40} intensity={7} color="lightblue" />
      <instancedMesh ref={mesh} args={[null, null, count]}>
        <dodecahedronBufferGeometry args={[0.08, 1]} />
